...
export default function DeleteAllTasksModal({
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setError(
        submitError instanceof Error ? submitError.message : "שגיאה במחיקת כל המשימות"
      );
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <Modal open={open} onClose={handleClose} className="max-w-md">
      <form onSubmit={handleSubmit} className="space-y-4 pt-2">
        <div>
          <h3 className="text-xl font-bold text-slate-800 dark:text-white">מחק את כל המשימות</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            פעולה זו תמחק לצמיתות את כל המשימות והתת-משימות. לא ניתן לבטל.
          </p>
        </div>

        <div className="rounded-lg border border-red-100 bg-red-50/60 p-3 dark:border-red-900/40 dark:bg-red-950/20">
          <p className="text-sm font-medium text-slate-800 dark:text-slate-100">
            {taskCount} משימות יימחקו מהלוח
          </p>
        </div>
